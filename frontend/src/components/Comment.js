...
const Comment = (id) => {
  ...
  const handleSubmit = async (event) => {
    ...
    dispatch(notify(`Created a new comment ${comment} for blog ${blogComments.title}`, 5))
    ...
    <div>
      ...
      <div>
        <form onSubmit={handleSubmit}>
          <div>
            <input data-testid = 'input' className='form' name='content' placeholder='More than 5 characters'/>
            <Button data-testid = 'commentBtn' size='sm' type='submit'>add comments</Button>
          </div>
        </form>
      </div>
      <div>
